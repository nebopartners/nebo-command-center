#!/bin/bash
# nebo-monitor.sh - Daemon that monitors all AI coding agent sessions
# and sends Discord/Telegram/WhatsApp notifications when approval is needed
#
# Usage: ./nebo-monitor.sh [--poll-interval SECONDS] [--reminder-interval SECONDS]
#
# Environment variables (optional - reads from ~/.openclaw/openclaw.json if not set):
#   OPENCLAW_TELEGRAM_ID    - Telegram chat ID (default: from openclaw config)
#   OPENCLAW_WEBHOOK_TOKEN  - Webhook auth token (default: from openclaw config)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Environment variables (optional - reads from ~/.openclaw/openclaw.json):"
            echo "  OPENCLAW_TELEGRAM_ID    Override Telegram chat ID"
            echo "  OPENCLAW_WEBHOOK_TOKEN  Override webhook auth token"
            exit 0
            ;;
        *)
