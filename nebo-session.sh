#!/bin/bash
# Claude Code Nebo - Complete wrapper for automated Claude Code sessions
...
MASTER_MONITOR="$SCRIPT_DIR/nebo-monitor.sh"
MONITOR_PID_FILE="/tmp/nebo-orchestrator/nebo-monitor.pid"
...
ensure_monitor_running() {
    ...
    echo "[Nebo] Starting master monitor daemon..."
    "$MASTER_MONITOR" > /dev/null 2>&1 &
    sleep 1

    if [ -f "$MONITOR_PID_FILE" ]; then
        echo "[Nebo] Master monitor started (PID: $(cat "$MONITOR_PID_FILE"))"
    fi
}
...
echo "[Nebo] Starting Claude Code session: $SESSION_NAME"
echo "[Nebo] Working directory: $WORKDIR"
...
echo "[Nebo] Waiting for Claude Code to initialize..."
sleep 5
...
echo ""
echo "You'll receive Discord notifications when approval is needed."
echo ""

# Monitor mode
if [ "$MONITOR" = true ]; then
    echo "[Nebo] Monitoring session (Ctrl+C to stop monitoring, session continues)..."
    echo ""
    ...
    echo "[Nebo] Session ended."
fi

# Wait mode
if [ "$WAIT" = true ] && [ "$MONITOR" = false ]; then
    echo "[Nebo] Waiting for session to complete..."
    while tmux has-session -t "$SESSION_NAME" 2>/dev/null; do
        sleep 5
    done
    echo "[Nebo] Session completed."
fi
